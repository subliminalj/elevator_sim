...
public:
	...
	void elevator::update(elevator& elev, int currTime)
	{
		//PICK UP PASSENGERS
		list<rider>::iterator wit = waiting_list.begin(); // init waiting list it
		for (wit = waiting_list.begin(); wit != waiting_list.end(); wit++) // cycle through waiting list
		{
			if (elev.floornum == wit->get_current_floor() && elev.get_up() == wit->get_up()) // if elevator is on a floor where someone is waiting and they are headed in the direction of the elevator, pick them up
			{
				elev.add_rider(*wit, currTime);
				wit->stop_wait_timer(currTime);
				wit->start_trip_timer(currTime);
			}
		}
		//UPDATE FLOOR FOR RIDERS -- this may not be neccessary
		list<rider>::iterator rit = rider_list.begin();
		...
		//DISEMBARK
		list<rider>::iterator dit = rider_list.begin();
